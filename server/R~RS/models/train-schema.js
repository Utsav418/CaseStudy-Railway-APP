...
var Trains = module.exports = mongoose.model('Trains', trainSchema, 'trains');


module.exports.getTrains = function(callback) {
    console.log("Requested getTrains() API 1");
    Trains.find(callback);
}

module.exports.getTrains = function(from, to, callback) {
    console.log("Requested getTrains() API 2");
    // console.log("From: " + from + "To: " + to);
    // console.log(callback);
    //Trains.find({src: from, dstn: to}, callback);
    Trains.find({ $or: [{ src: from, dstn: to }, { src: to, dstn: from }] }, callback).lean();
}
